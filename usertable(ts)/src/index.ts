...
// DOM elements
const rowsData = document.querySelector<HTMLTableSectionElement>(".body")!;
const userSearch = document.querySelector<HTMLInputElement>("#search")!;
const genderValue = document.querySelector<HTMLSelectElement>("#gender")!;
const gradeValue = document.querySelector<HTMLSelectElement>("#sort")!;
const rowValue = document.querySelector<HTMLSelectElement>("#btn")!;

let currentPage = 1;
let rowPerPage = 5;
let currentData: User[] = users;

// Pagination
function pagination(data: User[]) {
  currentData = data;
...
// Render buttons
function renderButtons(totalRecords: number) {
  const totalPages = Math.ceil(totalRecords / rowPerPage);
  const pageInfo = document.querySelector<HTMLDivElement>("#pagination")!;
...
// Render table
function renderFun(st: User[]) {
  let rows = "";
  const noFound = document.querySelector<HTMLDivElement>("#display")!;
...
function searchUser() {
  ...
  const filter = users.filter((curr) =>
    curr.name.toLowerCase().includes(userValue)
  );
  currentPage = 1;
  pagination(filter);
}
